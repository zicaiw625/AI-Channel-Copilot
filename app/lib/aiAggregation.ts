...
export const buildTopCustomers = (
  ...
  ordersInRange.forEach((order) => {
    ...
    prev.firstAIAcquired = prev.firstAIAcquired || Boolean(order.isNewCustomer && order.aiSource);
...
export const buildTrend = (
  ...
  ordersInRange.forEach((order) => {
    ...
    const bucketStart =
      bucket === "day"
        ? startOfDay(createdAt, timeZone)
        : bucket === "week"
          ? getWeekStart(createdAt, timeZone)
          : getMonthStart(createdAt, timeZone);
