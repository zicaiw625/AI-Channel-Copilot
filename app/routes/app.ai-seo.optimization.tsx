...
import { buildAiVisibilityHref, buildFunnelHref, getPreservedSearchParams } from "../lib/navigation";
...
export default function AIOptimization() {
  ...
  useEffect(() => {
    if (uiLanguage !== language) {
      ...
      navigateRef.current(
        {
          pathname: "/app/ai-seo/optimization",
  ...
  const backHref = buildAiVisibilityHref(location.search);
  const workspaceLlmsHref = buildAiVisibilityHref(location.search, { tab: "llms" });
  const backLabel = isEnglish ? "Back to AI SEO Workspace" : "返回 AI SEO 工作台";
