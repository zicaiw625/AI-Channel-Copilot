...
-- 优化 Webhook 队列处理
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_webhook_jobs_shop_status_next" 
ON "WebhookJob" ("shopDomain", "status", "nextRunAt") 
WHERE "status" IN ('queued', 'processing');

-- 优化 AI 源和日期范围的复合查询
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_orders_shop_currency_created" 
ON "Order" ("shopDomain", "currency", "createdAt" DESC);

-- 优化新客户统计
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_orders_new_customer_flag" 
ON "Order" ("shopDomain", "isNewCustomer", "aiSource") 
WHERE "isNewCustomer" = true;

-- 添加部分索引以节省空间和提升性能
CREATE INDEX CONCURRENTLY IF NOT EXISTS "idx_orders_ai_only" 
